      ...
      <motion.div whileHover={{ scale: 1.01 }} className="flex items-center gap-3 p-3 rounded-lg bg-card border border-border/30 hover:border-primary/30 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            {product.is_stockout ? <Badge variant="destructive" className="text-[10px] px-1 py-0">Sem estoque</Badge> : <span className="text-[10px] text-muted-foreground">{product.stock_quantity} un.</span>}
      ...
      <motion.div whileHover={{ y: -4 }} transition={{ type: 'spring', stiffness: 300 }}>
        <Card className="overflow-hidden border-border/30 hover:border-primary/30 transition-colors h-full flex flex-col">
          <div className="aspect-square relative bg-muted">
            ...
            {product.is_stockout && <div className="absolute inset-0 bg-background/80 flex items-center justify-center"><Badge variant="destructive">Esgotado</Badge></div>}
          ...
          <CardContent className="p-3 space-y-2 flex-1 flex flex-col">
            ...
            <div className="flex items-center justify-between">
              ...
              {!product.is_stockout && product.stock_quantity <= 10 && <Badge variant="outline" className="text-[10px] text-warning border-warning/50">{product.stock_quantity} un.</Badge>}
            </div>
            <div className="flex gap-1.5">
              <Button size="sm" variant="outline" className="flex-1 text-xs h-8" onClick={() => setShowDetails(true)}><Eye className="w-3.5 h-3.5 mr-1" />Detalhes</Button>
              {onSend && <Button size="sm" className="flex-1 text-xs h-8" onClick={() => onSend(product)} disabled={product.is_stockout}><Send className="w-3.5 h-3.5 mr-1" />Enviar</Button>}
            </div>
          </CardContent>
        </Card>
      </motion.div>
