...
// Expandable Section
interface ExpandableSectionProps { title: string; children: ReactNode; defaultOpen?: boolean; level?: 'basic' | 'intermediate' | 'advanced'; badge?: string; className?: string; }
  ...
  return (
    <div className="relative">
      ...
      <Button variant="ghost" size="sm" onClick={() => setIsExpanded(!isExpanded)} className="mt-2 w-full">
        <ChevronDown className={cn("w-4 h-4 mr-2 transition-transform", isExpanded && "rotate-180")} />{isExpanded ? hideLabel : showLabel}
      </Button>
    </div>
  );
}
    ...
    <div className="flex gap-2 p-1 bg-muted rounded-lg">
      {levels.map((l) => (
        <button key={l.value} onClick={() => setLevel(l.value)}
          ...
          <div className="font-medium">{l.label}</div><div className="text-xs text-muted-foreground">{l.description}</div>
        </button>
      ))}
    </div>
  );
