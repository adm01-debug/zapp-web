...
export function TourOverlay() {
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      switch (position) {
        case 'top': y = rect.top - tooltipHeight - padding - 12; break;
        case 'left': x = rect.left - tooltipWidth - padding - 12; y = rect.top + rect.height / 2 - tooltipHeight / 2; break;
        case 'right': x = rect.right + padding + 12; y = rect.top + rect.height / 2 - tooltipHeight / 2; break;
      }
    ...
    const handleResize = () => { rafId = requestAnimationFrame(updatePosition); };
    window.addEventListener('resize', handleResize);
    return () => { window.removeEventListener('resize', handleResize); clearTimeout(retryTimeout); cancelAnimationFrame(rafId); };
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      switch (e.key) {
        ...
        case 'ArrowRight': case 'Enter': nextStep(); break;
  ...
  return createPortal(
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[10000]">
          ...
          <defs>
            <mask id="spotlight-mask">
              ...
              <motion.rect initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                width={targetRect.width + padding * 2} height={targetRect.height + padding * 2} rx="12" fill="black" />
            </mask>
          </defs>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          style={{ left: targetRect.left - padding, top: targetRect.top - padding, width: targetRect.width + padding * 2, height: targetRect.height + padding * 2,
            boxShadow: '0 0 0 3px hsl(var(--primary)), 0 0 30px hsl(var(--primary) / 0.4)' }} />

        <motion.div className="absolute rounded-xl pointer-events-none border-2 border-primary/50"
          style={{ left: targetRect.left - padding, top: targetRect.top - padding, width: targetRect.width + padding * 2, height: targetRect.height + padding * 2 }}
          animate={{ scale: [1, 1.05, 1], opacity: [0.5, 0, 0.5] }} transition={{ duration: 2, repeat: Infinity }} />
          ...
          <div className="p-4 bg-gradient-to-r from-primary/10 to-secondary/10 border-b border-border/50">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-muted-foreground">Passo {currentStep + 1} de {steps.length}</span>
              </div>
              <Button variant="ghost" size="icon" className="h-7 w-7 -mr-1" onClick={endTour}><X className="w-4 h-4" /></Button>
            </div>
          </div>
          <div className="p-4">
          ...
          <div className="flex justify-center gap-1.5 pb-3">
            {steps.map((_, index) => (
              <motion.div key={index}
                ...
                whileHover={{ scale: 1.2 }} onClick={() => goToStep(index)} />
            ))}
          </div>
