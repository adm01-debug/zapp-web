...
const SUGGESTION_COMMANDS = [
  '"Abrir a inbox"', '"Mostrar o dashboard"', '"Buscar contato João"', '"Ir para equipe"',
];

export function VoiceSearchOverlay({ isOpen, phase, partialTranscript, finalTranscript, agentResponse, error, onClose, onStartListening, onStopListening, onStopSpeaking }: VoiceSearchOverlayProps) {
  ...
  return createPortal(
    <AnimatePresence mode="wait">
      {isOpen && <motion.div key="voice-overlay" className="fixed inset-0 z-[9999] flex items-center justify-center"
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: prefersReduced ? 0.1 : 0.3 }}
        role="dialog" aria-modal="true" aria-label="Assistente de voz">
        ...
        <motion.div className="relative z-10 flex flex-col items-center gap-5 p-8 rounded-3xl max-w-[340px] w-full mx-4 overflow-visible" style={{ background: 'transparent' }}
          ...
          <div className="relative z-10 flex flex-col items-center gap-5 w-full">
            ...
            <VoiceTranscriptArea phase={phase} partialTranscript={partialTranscript} finalTranscript={finalTranscript} agentResponse={agentResponse} error={error} />

            <AnimatePresence>
              {showSuggestions && phase === 'idle' && !agentResponse && (
                <motion.div initial={{ opacity: 0, y: prefersReduced ? 0 : 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: prefersReduced ? 0 : -8 }} transition={{ duration: 0.3 }} className="w-full space-y-2">
                  <p className="text-[10px] text-white/25 text-center uppercase tracking-widest font-semibold">Experimente dizer</p>
                  <div className="flex flex-col items-center gap-1.5">
                    {SUGGESTION_COMMANDS.map((cmd, i) => (
                      <motion.div key={cmd} initial={{ opacity: 0, y: prefersReduced ? 0 : 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: prefersReduced ? 0 : i * 0.06 }}
                        className="text-xs text-white/35 px-4 py-1.5 rounded-full border border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.06] hover:text-white/50 transition-colors cursor-default select-none">{cmd}</motion.div>
                    ))}
                  </div>
                </motion.div>
              )}
