...
import { ShieldQuestion, Lightbulb, Loader2, Sparkles, RefreshCw, AlertTriangle } from 'lucide-react';
...
Se não houver objeções, retorne []`,
      ...
      if (jsonMatch) {
        ...
        if (parsed.length > 0) {
          toast.success(`${parsed.length} objeção(ões) detectada(s)!`);
        }
      }
    } catch (err) {
    ...
    return (
      <div className="space-y-2.5">
        ...
        >
          ...
          {loading ? 'Analisando...' : `Detectar objeções (${lastMessages.length} msgs)`}
        </Button>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <div className="flex-1 h-1 rounded-full bg-muted/40 overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${confidenceColor(obj.confidence)}`} style={{ width: `${obj.confidence * 100}%` }} />
                      </div>
                      <span className="text-[9px] text-muted-foreground shrink-0">{confidenceLabel(obj.confidence)}</span>
                    </div>
                  </div>
                </div>
