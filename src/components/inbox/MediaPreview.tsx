...
import { X, Download, Play, Pause, FileText, File, FileSpreadsheet, FileImage, FileArchive, ExternalLink, Maximize, Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import { getFileExtension, formatFileSize } from '@/utils/whatsappFileTypes';
...
interface DocumentPreviewProps { url: string; fileName: string; fileSize?: number; isSent: boolean; }
...
export function DocumentPreview({ url, fileName, fileSize, isSent }: DocumentPreviewProps) {
  ...
  const handleDownload = async () => {
    ...
    toastFn.error('🔒 Download bloqueado por política de segurança', { description: 'O download de arquivos está desabilitado para proteção de dados.' });
  };

  const handleOpen = () => {
    import('sonner').then(({ toast }) => { toast.error('🔒 Abertura externa bloqueada por política de segurança'); });
  };

  return (
    <motion.div whileHover={{ scale: 1.02 }}
      className={cn("flex items-center gap-3 p-3 rounded-lg min-w-[240px] max-w-[300px] cursor-pointer transition-all",
        isSent ? "bg-primary-foreground/10 hover:bg-primary-foreground/15" : "bg-muted/50 hover:bg-muted/70 border border-border/30")}
      onClick={handleOpen}>
      <div className={cn("flex-shrink-0 w-12 h-12 rounded-lg flex items-center justify-center", isSent ? "bg-primary-foreground/20" : "bg-card")}>{getFileIcon(fileName)}</div>
      ...
      <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={(e) => { e.stopPropagation(); handleDownload(); }}
        className={cn("flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors",
      ...
      <div ref={ref}>
        <div className="space-y-2">
          <motion.div whileHover={{ scale: 1.02 }} className="relative rounded-lg overflow-hidden max-w-[300px] cursor-pointer" onClick={() => setShowFullscreen(true)}>
            {!isLoaded && <div className="absolute inset-0 bg-muted animate-pulse flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-muted-foreground" /></div>}
            <video src={url} className="w-full max-h-[200px] object-cover rounded-lg" muted={isMuted} loop playsInline
            ...
            <AnimatePresence>
              {!isPlaying && isLoaded && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-background/30 flex items-center justify-center">
                  <div className="w-12 h-12 rounded-full bg-background/90 flex items-center justify-center shadow-lg"><Play className="w-6 h-6 text-primary ml-0.5" /></div>
                </motion.div>
              )}
            </AnimatePresence>
        ...
        <AnimatePresence>{showFullscreen && <VideoFullscreen url={url} onClose={() => setShowFullscreen(false)} />}</AnimatePresence>
      </div>
    );
  }
);

export function StickerPreview({ url, isSent }: { url: string; isSent: boolean }) {
  const [isLoaded, setIsLoaded] = useState(false);
  return (
    <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }} className="relative">
      ...
      <motion.img src={url} alt="Sticker" onLoad={() => setIsLoaded(true)} whileHover={{ scale: 1.1 }}
        className={cn("max-w-[120px] max-h-[120px] object-contain", !isLoaded && "hidden")} />
    </motion.div>
  );
}
