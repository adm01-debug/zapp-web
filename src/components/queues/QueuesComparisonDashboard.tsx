...
export function QueuesComparisonDashboard() {
  ...
  const totals = queuesPerformance.reduce((acc, q) => ({
    contacts: acc.contacts + q.totalContacts, messages: acc.messages + q.totalMessages,
    agents: acc.agents + q.agentsCount, waiting: acc.waiting + q.waitingContacts,
  ...
  const summaryCards = [
    { icon: Users, color: 'primary', label: 'Total Contatos', value: totals.contacts },
    { icon: MessageSquare, color: 'success', label: 'Total Mensagens', value: totals.messages },
    { icon: TrendingUp, color: 'secondary', label: 'Total Atendentes', value: totals.agents },
    { icon: Clock, color: 'accent', label: 'Aguardando', value: totals.waiting },
  ] as const;
    ...
    <div className="p-6 space-y-6 overflow-y-auto h-full relative bg-background">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate('/')} className="hover:bg-muted/30"><ArrowLeft className="w-5 h-5" /></Button>
          <div>
            <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><BarChart3 className="w-6 h-6" />Comparação de Filas</h1>
      ...
      {queuesPerformance.length === 0 ? (
        ...
        <>
          ...
          <Card className="border border-secondary/20 bg-card/50 backdrop-blur">
            <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Users className="w-4 h-4" />Detalhamento por Fila</CardTitle></CardHeader>
            <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-border/20">
                      ...
                      <TableHead className="text-right">Atribuídos</TableHead><TableHead className="text-right">Aguardando</TableHead>
                      <TableHead className="text-right">Mensagens</TableHead><TableHead className="text-right">Média/Contato</TableHead>
                      <TableHead className="text-right">Atendentes</TableHead><TableHead className="text-right">Taxa Atrib.</TableHead><TableHead></TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {queuesPerformance.map((queue) => {
                      const assignmentRate = queue.totalContacts > 0 ? Math.round((queue.assignedContacts / queue.totalContacts) * 100) : 0;
                      return (
                        <TableRow key={queue.id} className="border-border/20 hover:bg-muted/10">
                          <TableCell><div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: queue.color }} /><span className="font-medium text-foreground">{queue.name}</span></div></TableCell>
                          ...
                          <TableCell className="text-right">
                            <Badge variant="secondary" className={assignmentRate >= 80 ? 'bg-success/10 text-success' : assignmentRate >= 50 ? 'bg-warning/10 text-warning' : 'bg-destructive/10 text-destructive'}>{assignmentRate}%</Badge>
                          </TableCell>
                          <TableCell><Button variant="ghost" size="icon" className="w-8 h-8" onClick={() => navigate(`/queue/${queue.id}`)}><Eye className="w-4 h-4" /></Button></TableCell>
                        </TableRow>
                      );
                    })}
                  </TableBody>
