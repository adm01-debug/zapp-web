/**
 * AdvancedCRMSearch — queries the external CRM database.
 */
...
import { Search, SlidersHorizontal, ArrowUpDown, Sparkles, Loader2, Filter, ChevronLeft, ChevronRight } from 'lucide-react';
...
function AdvancedCRMSearchInner({ onSelectContact, className }: AdvancedCRMSearchProps) {
  const {
    results, total, totalPages, currentPage, filters, isLoading, isFetching,
    hasActiveFilters, activeFilterCount, params, setSearch, setFilter,
    setSortBy, setPage, clearFilters, isConfigured,
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="outline" size="sm" className="h-9 gap-1.5 relative">
              <SlidersHorizontal className="w-4 h-4" /><span className="hidden sm:inline text-xs">Filtros</span>
              {activeFilterCount > 0 && <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-primary text-[10px] text-primary-foreground flex items-center justify-center">{activeFilterCount}</span>}
            </Button>
          </SheetTrigger>
          <SheetContent side="right" className="w-72 sm:w-80">
            ...
            <ScrollArea className="h-[calc(100vh-80px)] mt-4">
              <CRMFiltersPanel filters={filters} params={params} onFilter={setFilter as (key: string, value: string | boolean | undefined) => void} onClear={clearFilters} activeCount={activeFilterCount} />
            </ScrollArea>
          </SheetContent>
        </Sheet>
      ...
      <ScrollArea className="flex-1 min-h-0">
        <div className="p-3 space-y-2">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              <Search className="w-8 h-8 mx-auto mb-3 opacity-30" /><p className="text-sm">Nenhum resultado</p>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-3 py-2 border-t border-border/30 bg-muted/5">
          ...
          <div className="flex items-center gap-1">
            <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setPage(currentPage - 1)} disabled={currentPage === 0 || isFetching}><ChevronLeft className="w-3 h-3 mr-0.5" />Anterior</Button>
            <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setPage(currentPage + 1)} disabled={currentPage >= totalPages - 1 || isFetching}>Próxima<ChevronRight className="w-3 h-3 ml-0.5" /></Button>
          </div>
        </div>
      )}
    </div>
