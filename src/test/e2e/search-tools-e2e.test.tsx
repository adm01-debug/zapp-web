 ...
 * 5. useDebounce hook
 * 6. CommandPalette (fuzzyMatch, filtering, grouping)
...
import {
  ...
  useSmartDefaults,
  RecentSearches,
} from '@/components/cognitive/SmartDefaults';
...
function smartDefaultsWrapper({ children }: { children: React.ReactNode }) {
  return <SmartDefaultsProvider>{children}</SmartDefaultsProvider>;
}
...
describe('SearchInput — Component E2E', () => {
  ...
  it('shows clear button when value exists and not loading', () => {
    render(<SearchInput onChange={vi.fn()} value="hello" isLoading={false} showClear={true} />);
    expect(screen.getByText('Limpar busca')).toBeTruthy();
  });

  it('hides clear button when showClear=false', () => {
    render(<SearchInput onChange={vi.fn()} value="hello" showClear={false} />);
    expect(screen.queryByText('Limpar busca')).toBeNull();
  });

  it('calls onChange on clear click', () => {
    const onChange = vi.fn();
...
describe('useSearch — Hook E2E', () => {
  ...
  it('handles unicode/emoji in search term', () => {
    const { result } = renderHook(
      () => useSearch('contacts', { columns: ['name'] }),
      { wrapper: createQueryWrapper() }
    );
...
describe('useSearchHistory — Hook E2E', () => {
  ...
  it('persists to localStorage', () => {
    ...
    expect(JSON.parse(stored!)[0].query).toBe('persistido');
  });
...
// ═══════════════════════════════════════════════════════════
// 6. CommandPalette — Module Validation
// ═══════════════════════════════════════════════════════════
...
describe('SmartDefaults — RecentSearches E2E', () => {
  ...
  it('exports all search-related components', () => {
    expect(SmartDefaultsProvider).toBeDefined();
    expect(useSmartDefaults).toBeDefined();
    expect(RecentSearches).toBeDefined();
  });

  it('useSmartDefaults provides addRecentSearch', () => {
    const { result } = renderHook(() => useSmartDefaults(), { wrapper: smartDefaultsWrapper });
  ...
  it('clearHistory resets all defaults', () => {
    const { result } = renderHook(() => useSmartDefaults(), { wrapper: smartDefaultsWrapper });
    act(() => {
...
describe('InboxFilters — State Shape E2E', () => {
  it('exports InboxFilters component', async () => {
...
describe('Search Integration — Cross-Component', () => {
  ...
  it('keyboard module exports are consistent', () => {
